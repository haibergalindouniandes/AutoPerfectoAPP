...
class Vista_lista_mantenimientos(QWidget):
    ...
    def inicializar_GUI(self):
        ...
        self.btn_volver.setStyleSheet("border :1px solid black;")
        ...
        # setting up the border
        self.btn_aniadir_mantenimiento.setStyleSheet("border :1px solid black;")
    ...
    def mostrar_mantenimientos(self, mantenimientos):
        ...
        for mantenimiento in self.mantenimientos:
            ...
            # setting up the border
            boton_editar.setStyleSheet("border :1px solid black;")
            ...
            # setting up the border
            etiqueta_eliminar.setStyleSheet("border :1px solid black;")
            etiqueta_eliminar.setFixedSize(30,30)
            etiqueta_eliminar.setIcon(QIcon("src/recursos/005-delete.png"))
            etiqueta_eliminar.clicked.connect(partial(self.eliminar_mantenimiento, mantenimiento["Nombre"]) )
    ...
    def error_mantenimiento_existe(self):
            mensaje_error=QMessageBox()
            mensaje_error.setStyleSheet(self.constante.COLOR_INTERFAZ + self.constante.COLOR_TEXTO)
            mensaje_error.setIcon(QMessageBox.Critical)
            mensaje_error.setText("El mantenimiento ya se encuentra registradon\nPor favor verifique e intente nuevamente.")        
            mensaje_error.setWindowTitle("Error al crear mantenimiento")
            mensaje_error.setWindowIcon(QIcon("src/recursos/smallLogo.png"))
            mensaje_error.setStandardButtons(QMessageBox.Ok ) 
            respuesta=mensaje_error.exec_()    

    def error_mantenimiento_no_existe(self):
            mensaje_error=QMessageBox()
            mensaje_error.setStyleSheet(self.constante.COLOR_INTERFAZ + self.constante.COLOR_TEXTO)
            mensaje_error.setIcon(QMessageBox.Critical)
            mensaje_error.setText("El mantenimiento no se encuentra registrado\nPor favor verifique e intente nuevamente.")        
            mensaje_error.setWindowTitle("Error al crear mantenimiento")
            mensaje_error.setWindowIcon(QIcon("src/recursos/smallLogo.png"))
            mensaje_error.setStandardButtons(QMessageBox.Ok ) 
            respuesta=mensaje_error.exec_()              